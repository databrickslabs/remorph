...
The **Remorph Analyzer** is built to scan and interpret metadata from ETL pipelines and SQL assets. Its analysis provides several key insights:
...
```bash
 databricks labs remorph analyze --help
 ```
...
```bash
 databricks labs remorph analyze --source-directory <absolute-path> --report-file <absolute-path>
```

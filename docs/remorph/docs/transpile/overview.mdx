...
**Bladerunner** is a flexible and extensible code conversion engine designed to accelerate modernization to Databricks. It supports a wide range of **ETL platforms** (e.g., Informatica, DataStage) and **SQL-based systems** (e.g., Oracle, Teradata, Netezza), accepting inputs in the form of exported metadata and scripts.
...
Bladerunner is fully **extensible** — new converters can be added, and the output structure can be **customized** to meet project-specific requirements.

Whether you're migrating a single job or thousands, Bladerunner delivers predictable, repeatable results optimized for the Databricks ecosystem.

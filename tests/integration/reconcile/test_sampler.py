...
def test_random_sampler_count(
    mock_spark
):
    keys_df = mock_spark.createDataFrame(
        [
            Row(key=1),
            Row(key=2),
            Row(key=3),
    ...
    assert random_sample.count() > 0

def test_random_sampler_negative_count(
    mock_spark
):
    keys_df = mock_spark.createDataFrame(
        [
...
def test_stratified_sampler_count(
    mock_spark
):
    keys_df = mock_spark.createDataFrame(
        [
    ...
    assert stratified_sample.count() > 0

def test_stratified_sampler_missing_columns_buckets(
):
    with pytest.raises(ValueError, match="SamplingOptions : "
...
def test_stratified_sampler_negative_count(
    mock_spark
):
    keys_df = mock_spark.createDataFrame(
        [

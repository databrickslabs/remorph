...
path_expected_exceptions = Path(__file__).parent / Path('../../resources/functional/presto_expected_exceptions/')
functional_tests_expected_exceptions = get_functional_test_files_from_directory(
    path_expected_exceptions, "presto", "databricks", True
)
...
def test_presto_expected_exceptions(dialect_context, sample: FunctionalTestFileWithExpectedException):

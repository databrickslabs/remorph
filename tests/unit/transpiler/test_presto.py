...
path = Path(__file__).parent / Path('../../resources/functional/presto/')
functional_tests = get_functional_test_files_from_directory(path, "presto", "databricks", False)
test_names = [f.test_name for f in functional_tests]


@pytest.mark.parametrize("sample", functional_tests, ids=test_names)
def test_presto(dialect_context, sample: FunctionalTestFile):

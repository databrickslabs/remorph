...
from databricks.labs.remorph.helpers.reconcile_utils import (
    ReconcileConfigUtils,
)
from databricks.labs.remorph.reconcile.recon_config import Table
from databricks.sdk.errors.platform import ResourceDoesNotExist
from databricks.sdk.service.workspace import SecretScope, GetSecretResponse
    ...
    return MockInstallation(
        {
            ...
            "recon_conf_snowflake.json": {
                "source_schema": "sfs",
                "source_catalog": "sfc",
                "target_catalog": "dbc",
                "target_schema": "dbs",
                "tables": [
                    {"source_name": "odc", "target_name": "odc"},
                ],
            },
        }
    )
...
def test_configure_secrets_oracle_insert(mock_workspace_client, mock_installation):
    ...
    mock_workspace_client.secrets.list_scopes.side_effect = [[SecretScope(name=SCOPE_NAME)]]
    mock_workspace_client.secrets.get_secret.return_value = [[GetSecretResponse(key="source", value="dummy")]]
...
def test_recon_config_prompt_tables_list_include_save_config_details(
    mock_workspace_client, mock_installation, monkeypatch
):
    def mock_open(url):
...
def test_recon_config_prompt_tables_list_exclude_save_config_details(
    mock_workspace_client, mock_installation, monkeypatch
):
    def mock_open(url):
        print(f"Opening URL: {url}")
    ...
    prompts = MockPrompts(
        {
            ...
            r".*Did you setup the secrets for the": "yes",
            r"Would you like to run reconciliation.*": "no",
            r"Would you like to run reconciliation on `all` tables.*": 1,
            r"Comma-separated list of tables to `exclude`": "table1, table2",
            r"Open.* config file in the browser?": "yes",
            r"Open `README_RECON_CONFIG` setup instructions in your browser?": "yes",
        }
    )

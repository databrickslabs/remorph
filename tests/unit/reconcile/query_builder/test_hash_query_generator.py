...
def test_hash_query_generator(app_config):
    actual_query = HashQueryGenerator(
        source_type=SourceType.ORACLE.value, table_conf=app_config[0], schema=app_config[1], layer="source"
    ).generate_hash_query()
    expected_query = (
        "select lower(RAWTOHEX(STANDARD_HASH(coalesce(trim(s_suppkey),'') || trim(s_name) "
        "|| trim(s_address) || coalesce(trim(s_nationkey),'') || coalesce(trim(s_phone),'') "
        "|| trim(to_char(s_acctbal, '9999999999.99')) || trim(s_comment), 'SHA256'))) "
        "as hash_value__recon , s_suppkey from supplier"
    )

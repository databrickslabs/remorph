...
from databricks.labs.remorph.reconcile.query_builder.adapter import (
    QueryBuilderAdapterFactory,
)
...
def oracle_query_builder():
    table_conf = Tables(
        source_name="test_emp",
        target_name="test_emp",
        jdbc_reader_options=JdbcReaderOptions(
            number_partitions=2, partition_column="id", lower_bound="0", upper_bound="100"
        ),
...
def test_build_sql_query(oracle_query_builder):
    ...
    expected = (
        "select lower(RAWTOHEX(STANDARD_HASH(coalesce(trim(id),'') || coalesce(trim(name),'') "
        "|| coalesce(trim(sal),''), 'SHA256'))) as hash_value__recon , id from test_emp"
    )

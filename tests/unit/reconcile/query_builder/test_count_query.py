...
def test_count_query(table_mapping_with_opts):
    source_query = CountQueryBuilder(
        table_mapping=table_mapping_with_opts, layer="source", engine=get_dialect("oracle")
    ).build_query()
    target_query = CountQueryBuilder(
        table_mapping=table_mapping_with_opts, layer="target", engine=get_dialect("databricks")
    ).build_query()

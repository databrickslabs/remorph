...
def test_table_without_join_column(table_mapping_factory):
    ...
    assert mapping.get_join_columns(Layer.SOURCE) is None
    assert mapping.get_drop_columns(Layer.SOURCE) == set()
    assert mapping.get_partition_column(Layer.SOURCE) == set()
    assert mapping.get_partition_column(Layer.TARGET) == set()
    assert mapping.get_filter(Layer.SOURCE) is None
    assert mapping.get_filter(Layer.TARGET) is None
    assert mapping.get_threshold_columns(Layer.SOURCE) == set()
...
def test_table_without_column_mapping(table_mapping_factory, column_mapping):
    ...
    assert mapping.get_layer_tgt_to_src_col_mapping("s_address_t", Layer.TARGET) == "s_address_t"
    assert mapping.get_layer_tgt_to_src_col_mapping("s_address", Layer.SOURCE) == "s_address"
    assert mapping.get_src_to_tgt_col_mapping_list(["s_address", "s_name"], Layer.SOURCE) == {"s_address", "s_name"}
    assert mapping.get_src_to_tgt_col_mapping_list(["s_address", "s_name"], Layer.TARGET) == {"s_address", "s_name"}
    assert mapping.get_layer_src_to_tgt_col_mapping("s_address", Layer.SOURCE) == "s_address"

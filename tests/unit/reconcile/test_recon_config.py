...
def test_table_with_all_options(table_conf_with_opts):
    ...
    assert table_conf_with_opts.get_join_columns(Layer.SOURCE) == {"s_nationkey", "s_suppkey"}
    assert table_conf_with_opts.get_drop_columns(Layer.SOURCE) == {"s_comment"}
    assert table_conf_with_opts.get_partition_column(Layer.SOURCE) == {"s_nationkey"}
    assert table_conf_with_opts.get_partition_column(Layer.TARGET) == set()
    assert table_conf_with_opts.get_filter(Layer.SOURCE) == "s_name='t' and s_address='a'"
    assert table_conf_with_opts.get_threshold_columns(Layer.SOURCE) == {"s_acctbal"}
    ...
    assert table_conf_with_opts.get_partition_column(Layer.TARGET) == set()
    assert table_conf_with_opts.get_filter(Layer.TARGET) == "s_name='t' and s_address_t='a'"

...
def test_table_without_join_column(table_conf_mock):
    ...
    assert table_conf.join_columns == set()
    assert table_conf.drop_columns == set()
    assert table_conf.partition_column("source") == set()
    assert table_conf.partition_column("target") == set()
    assert table_conf.get_filter("source") == " 1 = 1 "
    assert table_conf.get_filter("target") == " 1 = 1 "
    assert table_conf.threshold_columns == set()
...
def test_table_with_all_options(table_conf_with_opts):
    ...
    assert table_conf.join_columns == {"s_suppkey"}
    assert table_conf.drop_columns == {"s_comment"}
    assert table_conf.partition_column("source") == {"s_nationkey"}
    assert table_conf.partition_column("target") == set()
    ...
    assert table_conf.threshold_columns == {"s_acctbal"}

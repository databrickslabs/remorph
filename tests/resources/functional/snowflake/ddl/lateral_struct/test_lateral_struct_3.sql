...
FROM (
  SELECT
    STRUCT(
      ...
      ARRAY(
        STRUCT('some_attributes' AS attributes, 12.34 AS prop),
        STRUCT('other_attributes' AS attributes, 56.78 AS prop)
      ) AS impressions,
      'some_candidates' AS candidates
    ) AS col,
    '2024-08-28' AS event_date,
    'store.replacements_view' AS event_name
) AS d
LATERAL VIEW OUTER EXPLODE(d.col.impressions) AS i
WHERE
  d.event_date = '2024-08-28'
  AND d.event_name IN ('store.replacements_view');
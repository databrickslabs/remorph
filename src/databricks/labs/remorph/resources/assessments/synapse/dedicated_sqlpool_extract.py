...
def execute():
    ...
    try:
        ...
        for idx, sqlpool_name in enumerate(sqlpool_names_to_profile):
            print(f"{'*'*60}")
            ...
            result = connection.execute(text(session_query))
            save_resultset_to_db(result, table_name, db_path, mode="append")

            table_name = "session_request"
            prev_max_end_time = get_max_column_value_duckdb("end_time", table_name, db_path)
            session_query = SynapseQueries.list_sessions(prev_max_end_time)
            result = connection.execute(text(session_query))
            save_resultset_to_db(result, table_name, db_path, mode="append")

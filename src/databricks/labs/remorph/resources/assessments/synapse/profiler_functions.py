...
def get_sqlpool_reader(sql_endpoint, user, password):
    ...
    '''
    return spark.read.format("jdbc").option("url", sql_endpoint).option("user", user).option("password", password)
...
def get_dedicated_sqlpool_reader(db_name):
    ...
    returns a jdbc reader for the given dedicated SQL Pool database
    '''
    sql_endpoint = synapse_jdbc["dedicated_sqlpool_url_template"].format(
        endpoint=synapse_workspace["dedicated_sql_endpoint"], database=db_name
    )
...
def get_serverless_sqlpool_reader():
    ...
    '''
    sql_endpoint = synapse_jdbc["serverless_sqlpool_url_template"].format(
        endpoint=synapse_workspace["serverless_sql_endpoint"]
    )
    user = synapse_workspace["sql-user"]
    password = synapse_workspace["sql-password"]
    return get_sqlpool_reader(sql_endpoint, user, password)
...
def dbfs_to_linux_path(dbfs_path):
    return re.sub("^dbfs:", "", re.sub("^(?!((dbfs:/Volumes)|(/Volumes)))((dbfs:)?)", "/dbfs", dbfs_path))
...
def get_synapse_artifacts_client():
    ...
    '''
    return ArtifactsClient(endpoint=synapse_workspace["development_endpoint"], credential=DefaultAzureCredential())
...
def trim_sql_comments(input_sql):
    ...
    input_with_no_blockandline_comments = "\n".join(
        [re.sub(r'^\s*(--).*', '', line) for line in input_with_no_block_comments.split("\n")]
    )

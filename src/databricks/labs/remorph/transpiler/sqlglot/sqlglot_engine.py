...
class SqlglotEngine:
    ...
    def partial_transpile(
        ...
        for error in errors:
            transpiled_sql_statements.append(error)
            error_list.append(ParserError(file_name, refactor_hexadecimal_chars(str(error))))
        return transpiled_sql_statements, error_list
    ...
    def transpile(
        ...
        try:
            ...
            transpiled_sql, error = self.partial_transpile(write_dialect, sql, file_name, error_list)
    ...
    @staticmethod
    def safe_parse(statements: str, read: Dialect) -> tuple[list[exp.Expression], list[str]]:
        dialect = read
        errors = []
        try:

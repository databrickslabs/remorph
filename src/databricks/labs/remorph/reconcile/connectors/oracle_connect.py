...
from databricks.labs.remorph.reconcile.constants import SourceDriver, Constants, SourceType
...
class OracleAdapter(SourceAdapter):
    ...
    def extract_schema(self, database_conf: DatabaseConfig, table_conf: Tables) -> list[Schema]:
        try:
            ...
            """
            df = (self.__get_oracle_reader(query).load().withColumn("CHAR_LENGTH", expr("cast(CHAR_LENGTH as int)"))
            ...
            raise Exception(
                ...
                f"OracleAdapter.extract_schema(): {str(e)}")
    ...
    def extract_data(self, table_conf: Tables, query: str) -> DataFrame:
        try:
            if table_conf.jdbc_reader_options is None:
                ...
                df = (self.__get_oracle_reader(query)
            ...
            raise Exception(
                f"An error occurred while executing Oracle SQL query {query} in OracleAdapter.extract_data(): {str(e)}")

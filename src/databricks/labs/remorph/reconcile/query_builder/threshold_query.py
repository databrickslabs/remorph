...
class ThresholdQueryBuilder(QueryBuilder):
    ...
    def build_comparison_query(self) -> str:
        self._validate(
            self.table_mapping.get_join_columns("source"), "Join Columns are compulsory for threshold comparison query"
        )
        join_columns = (
            self.table_mapping.get_join_columns("source") if self.table_mapping.get_join_columns("source") else set()
        )

...
_RECON_README_URL = "https://github.com/databrickslabs/remorph/blob/main/docs/README_RECON.md"
...
class ReconcileRunner:
    ...
    def run(self):
        ...
        wait = self._context.workspace_client.jobs.run_now(job_id)
        if not wait.run_id:
            raise SystemExit(f"Job {job_id} execution failed. Please check the job logs for more details.")

        job_run_url = f"{self._context.connect_config.host}/jobs/{job_id}/runs/{wait.run_id}"
        logger.info(f"Reconcile job started. Please check the job_url `{job_run_url}` for the current status.")
        if self._context.prompts.confirm(f"Would you like to open the job run URL `{job_run_url}` in the browser?"):
            webbrowser.open(job_run_url)

    def _get_verified_recon_config(self) -> ReconcileConfig:
        try:
            recon_config = self._context.recon_config
            if not recon_config:
                raise SystemExit("Cannot find existing `reconcile` installation. Please try reinstalling.")
    ...
    def _verify_recon_table_config(self, recon_config):
        ...
        try:
            ...
            self._context.installation.load(TableRecon, filename=filename)
    ...
    def _get_recon_job_id(self, reconcile_config: ReconcileConfig) -> int:
        ...
        if RECON_JOB_NAME in self._context.install_state.jobs:
            logger.debug("Reconcile job id found in the install state.")
            return int(self._context.install_state.jobs[RECON_JOB_NAME])

...
from sqlglot.expressions import Column, DataType
...
from databricks.labs.remorph.reconcile.query_builder_refactored.expression_generator import (
    build_alias, json_format, DataType_transform_mapping, default_transformer,
)
...
class SamplingQueryBuilder(QueryBuilder):
    ...
    def build_query(self, df: DataFrame):
        ...
        col_with_alias = [build_alias(this=col, alias=self.table_conf.get_col_mapping(col, self.layer)) for col in
                          cols]

        sql = select(*col_with_alias).sql()
        if self.custom_transformations:
            default_schema = {key: self.schema_dict[key] for key in self.schema_dict.keys() if
                              key not in self.custom_transformations}

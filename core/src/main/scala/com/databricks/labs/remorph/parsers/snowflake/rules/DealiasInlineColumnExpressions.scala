...
class DealiasInlineColumnExpressions extends Rule[LogicalPlan] with TransformationConstructors {

  override def apply(plan: LogicalPlan): Transformation[LogicalPlan] = plan.transform { case WithCTE(ctes, query) =>
    bundleInlineColumns(ctes, query)
  }

  private def bundleInlineColumns(plans: Seq[LogicalPlan], query: LogicalPlan): Transformation[LogicalPlan] = {
    ...
    query
      .transformUp { case p =>
        p.transformExpressionsUp {
          case Column(None, id: Id) if columnNamesToValues.contains(id) => ok(columnNamesToValues(id))
          case id: Id if columnNamesToValues.contains(id) => ok(columnNamesToValues(id))
        }
      }
      .map(WithCTE(tables, _))

...
class DefineJob extends Rule[JobNode] with TransformationConstructors {
  override def apply(tree: JobNode): Transformation[JobNode] = tree transformUp {
    case NeedsVariables(SuccessPy(name, code), variables) =>
      ok(PythonNotebookTask(CreatedFile(s"notebooks/$name.py", code), variables.map(_ -> "FILL_ME").toMap))
    case SuccessPy(name, code) =>
      ok(PythonNotebookTask(CreatedFile(s"notebooks/$name.py", code)))
    case NeedsVariables(SuccessSQL(name, code), variables) =>
      ok(SqlNotebookTask(CreatedFile(s"notebooks/$name.sql", code), variables.map(_ -> "FILL_ME").toMap))
    case SuccessSQL(name, code) =>
      ok(SqlNotebookTask(CreatedFile(s"notebooks/$name.sql", code)))
    case m: Migration =>

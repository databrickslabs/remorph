...
class TryGeneratePythonNotebook(generator: PySparkGenerator) extends Rule[JobNode] {
  override def apply(tree: JobNode): JobNode = tree transformDown {
    case n @ QueryPlan(plan, query) =>
      val state = Generating(plan, n, 0, 0, GeneratorContext(new LogicalPlanGenerator))

...
case class CreateArray(children: Seq[Expression], useStringTypeWhenEmpty: Boolean = false) extends Expression with Fn {
  ...
  override def dataType: DataType = ArrayType(children.headOption.map(_.dataType)
...
/** element_at(array, index) - Returns element of array at given (1-based) index. If index < 0,
 ...
 if the key is not contained in the map and `spark.sql.ansi.enabled` is set to false.
 If `spark.sql.ansi.enabled` is set to true, it throws NoSuchElementException instead. */
case class ElementAt(left: Expression, right: Expression, c: c) extends Expression with Fn {
  override def prettyName: String = "ELEMENT_AT"
  override def children: Seq[Expression] = Seq(left, right, c)
  override def dataType: DataType = UnresolvedType
}
...
case class Elt(children: Seq[Expression], right: right) extends Expression with Fn {
...
/** exists(expr, pred) - Tests whether a predicate holds for one or more elements in the array. */
case class ArrayExists(left: Expression, right: Expression, c: c) extends Expression with Fn {
  override def prettyName: String = "EXISTS"
  override def children: Seq[Expression] = Seq(left, right, c)
  override def dataType: DataType = UnresolvedType
}
...
case class CsvToStructs(left: Expression, right: Expression, c: c) extends Expression with Fn {
...
/** from_csv(csvStr, schema[, options]) - Returns a struct value with the given `csvStr` and `schema`. */
case class CsvToStructs(left: Expression, right: Expression) extends Binary(left, right) with Fn {
  override def prettyName: String = "FROM_CSV"
  override def dataType: DataType = UnresolvedType
}
...
/** from_csv(csvStr, schema[, options]) - Returns a struct value with the given `csvStr` and `schema`. */
case class CsvToStructs(left: left, right: right, c: Expression, d: d) extends Expression with Fn {
  override def prettyName: String = "FROM_CSV"
...
/** hash(expr1, expr2, ...) - Returns a hash value of the arguments. */
case class Murmur3Hash(children: Seq[Expression], right: right) extends Expression with Fn {
  override def prettyName: String = "HASH"
  override def children: Seq[Expression] = Seq(children, right)
  override def dataType: DataType = UnresolvedType
}
...
case class In(left: Expression, children: Seq[Expression]) extends Expression with Fn {
  override def prettyName: String = "IN"
  override def children: Seq[Expression] = Seq(left, children)
  override def dataType: DataType = UnresolvedType
}
...
case class MakeInterval(left: Expression, right: Expression, c: Expression, d: Expression, e: Expression) extends Expression with Fn {
...
/** make_interval(years, months, weeks, days, hours, mins, secs) - Make interval from years, months, weeks, days, hours, mins and secs. */
case class MakeInterval(left: Expression, right: Expression, c: Expression, d: Expression, e: Expression, f: Expression) extends Expression with Fn {
  override def prettyName: String = "MAKE_INTERVAL"
...
/** make_interval(years, months, weeks, days, hours, mins, secs) - Make interval from years, months, weeks, days, hours, mins and secs. */
case class MakeInterval(left: Expression, right: Expression, c: Expression, d: Expression, e: Expression, f: Expression, g: Expression) extends Expression with Fn {
  override def prettyName: String = "MAKE_INTERVAL"
...
/** approx_percentile(col, percentage [, accuracy]) - Returns the approximate `percentile` of the numeric
 ...
 In this case, returns the approximate percentile array of column `col` at the given
 percentage array. */
case class ApproximatePercentile(left: Expression, right: Expression, c: Expression, d: d, e: e) extends Expression with Fn {
  override def prettyName: String = "APPROX_PERCENTILE"
  override def children: Seq[Expression] = Seq(left, right, c, d, e)
  override def dataType: DataType = UnresolvedType
}
...
case class CountMinSketchAgg(left: Expression, right: Expression, c: Expression, d: Expression) extends Expression with Fn {
...
/** count_min_sketch(col, eps, confidence, seed) - Returns a count-min sketch of a column with the given esp,
 confidence and seed. The result is an array of bytes, which can be deserialized to a
      `CountMinSketch` before usage. Count-min sketch is a probabilistic data structure used for
 cardinality estimation using sub-linear space. */
case class CountMinSketchAgg(left: Expression, right: Expression, c: Expression, d: Expression, e: e, f: f) extends Expression with Fn {
  override def prettyName: String = "COUNT_MIN_SKETCH"
  override def children: Seq[Expression] = Seq(left, right, c, d, e, f)
  override def dataType: DataType = UnresolvedType
}
...
case class MakeTimestamp(left: Expression, right: Expression, c: Expression, d: Expression, e: Expression, f: Expression, g: g) extends Expression with Fn {
...
/** make_timestamp(year, month, day, hour, min, sec[, timezone]) - Create timestamp from year, month, day, hour, min, sec and timezone fields. */
case class MakeTimestamp(left: Expression, right: Expression, c: Expression, d: Expression, e: Expression, f: Expression, g: Expression) extends Expression with Fn {
  override def prettyName: String = "MAKE_TIMESTAMP"
  override def children: Seq[Expression] = Seq(left, right, c, d, e, f, g)
  override def dataType: DataType = UnresolvedType
}
...
/** make_timestamp(year, month, day, hour, min, sec[, timezone]) - Create timestamp from year, month, day, hour, min, sec and timezone fields. */
case class MakeTimestamp(left: Expression, right: Expression, c: Expression, d: Expression, e: Expression, f: Expression) extends Expression with Fn {
  override def prettyName: String = "MAKE_TIMESTAMP"
  override def children: Seq[Expression] = Seq(left, right, c, d, e, f)
  override def dataType: DataType = UnresolvedType
}

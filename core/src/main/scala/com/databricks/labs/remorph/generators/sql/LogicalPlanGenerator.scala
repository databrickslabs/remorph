...
class LogicalPlanGenerator(
  ...
  private def constraint(ctx: GeneratorContext, c: ir.Constraint): SQL = c match {
    ...
    case ir.GeneratedAlways(expression) => sql"GENERATED ALWAYS AS (${expr.generate(ctx, expression)})"
  }

  private def generateIdentityConstraint(c: ir.IdentityConstraint): SQL = c match {
    case ir.IdentityConstraint(None, None, true, false) => sql"GENERATED ALWAYS AS IDENTITY"
    case ir.IdentityConstraint(None, None, false, true) => sql"GENERATED BY DEFAULT AS IDENTITY"
    case ir.IdentityConstraint(Some(seed), Some(step), false, true) =>
      sql"GENERATED BY DEFAULT AS IDENTITY (START WITH $seed INCREMENT BY $step)"
    case ir.IdentityConstraint(Some(seed), None, false, true) =>
      sql"GENERATED BY DEFAULT AS IDENTITY (START WITH $seed)"
    case ir.IdentityConstraint(None, Some(step), false, true) =>
      sql"GENERATED BY DEFAULT AS IDENTITY (INCREMENT BY $step)"

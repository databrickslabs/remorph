...
class TSqlCallMapper extends CallMapper with TransformationConstructors {

  override def convert(call: Fn): Transformation[Expression] = {
    call match {
      case CallFunction("DATEADD", args) =>
        processDateAdd(args)
      case CallFunction("GET_BIT", args) => ok(BitwiseGet(args.head, args(1)))
      case CallFunction("SET_BIT", args) => ok(genBitSet(args))
      case CallFunction("CHECKSUM_AGG", args) => ok(checksumAgg(args))
  ...
  private def processDateAdd(args: Seq[Expression]): Transformation[Expression] = {
    ...
    args.head match {
      case Column(_, Id(id, _)) =>
        id.toLowerCase() match {
          ...
          case "hour" | "hh" => ok(Add(objectReference, KnownInterval(value, HOUR_INTERVAL)))

          // Minutes are not supported in Spark SQL, but we can use the number of minutes to create an INTERVAL
          // and add it to the object reference
          case "minute" | "mi" | "n" => ok(Add(objectReference, KnownInterval(value, MINUTE_INTERVAL)))

          // Seconds are not supported in Spark SQL, but we can use the number of seconds to create an INTERVAL
          // and add it to the object reference
          case "second" | "ss" | "s" => ok(Add(objectReference, KnownInterval(value, SECOND_INTERVAL)))

          // Milliseconds are not supported in Spark SQL, but we can use the number of milliseconds to create
          // an INTERVAL and add it to the object reference
          case "millisecond" | "ms" => ok(Add(objectReference, KnownInterval(value, MILLISECOND_INTERVAL)))

          // Microseconds are not supported in Spark SQL, but we can use the number of microseconds to create
          // an INTERVAL and add it to the object reference
          case "microsecond" | "mcs" => ok(Add(objectReference, KnownInterval(value, MICROSECOND_INTERVAL)))

          // Nanoseconds are not supported in Spark SQL, but we can use the number of nanoseconds to create an INTERVAL
          // and add it to the object reference
          case "nanosecond" | "ns" => ok(Add(objectReference, KnownInterval(value, NANOSECOND_INTERVAL)))

          case itv =>
            lift(
              PartialResult(
                args.head,
                SimpleError(s"DATEADD interval type '${itv}' is not valid. Should be 'day', 'month', 'year', etc.")))
        }
      case x =>
        lift(
          PartialResult(x, SimpleError("DATEADD interval type is not valid. Should be 'day', 'month', 'year', etc.")))
    }

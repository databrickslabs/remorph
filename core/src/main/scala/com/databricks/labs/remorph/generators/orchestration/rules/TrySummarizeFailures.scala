...
class TrySummarizeFailures extends Rule[JobNode] with TransformationConstructors {

  override def apply(tree: JobNode): Transformation[JobNode] = {
    var partials = Seq.empty[(ExecutedQuery, String)]
    tree
      .transformUp { case PartialQuery(executed, message, query) =>
        partials = partials :+ ((executed, message))
        ok(query)
      }
      .flatMap { removedPartials =>

...
class TSqlAstBuilder(override val vc: TSqlVisitorCoordinator)
  ...
  override def visitTSqlFile(ctx: TSqlParser.TSqlFileContext): ir.WithKnownLocationRange[ir.LogicalPlan] = {
  ...
  private def buildBatch(ctx: TSqlParser.BatchContext): Seq[ir.WithKnownLocationRange[ir.LogicalPlan]] = {
    // This very top level visitor does not ignore any valid statements for the batch, instead
    // we prepend any errors to the batch plan, so they are generated first in the output.
    val errors = errorCheck(ctx)
    val executeBodyBatchPlan = Option(ctx.executeBodyBatch()).map(_.accept(this))
    val sqlClausesPlans =
      ctx.sqlClauses().asScala.map(_.accept(this)).collect { case p: ir.WithKnownLocationRange[ir.LogicalPlan] => p }
    ...
    errors match {
      case Some(errorResult) => errorResult.asInstanceOf[ir.WithKnownLocationRange[ir.LogicalPlan]] +: validStatements
      case None => validStatements
    }
  }

  // TODO: Stored procedure calls etc as batch start
  override def visitExecuteBodyBatch(
      ctx: TSqlParser.ExecuteBodyBatchContext): ir.WithKnownLocationRange[ir.LogicalPlan] = {
    withLocationRangeFromRuleContext(ctx) {
  ...
  override def visitSqlClauses(ctx: TSqlParser.SqlClausesContext): ir.WithKnownLocationRange[ir.LogicalPlan] = {
    withLocationRangeFromRuleContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
          ctx match {
            ...
            case coaTrigger if coaTrigger.createOrAlterTrigger() != null =>
              coaTrigger.createOrAlterTrigger().accept(this)
  ...
  override def visitDmlClause(ctx: TSqlParser.DmlClauseContext): ir.WithKnownLocationRange[ir.LogicalPlan] = {
    withLocationRangeFromRuleContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>

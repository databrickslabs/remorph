...
class TSqlRelationBuilder(override val vc: TSqlVisitorCoordinator)
    extends TSqlParserBaseVisitor[ir.WithKnownOrigin[ir.LogicalPlan]]
  ...
  override def visitCommonTableExpression(ctx: CommonTableExpressionContext): ir.WithKnownOrigin[ir.LogicalPlan] =
  ...
  override def visitSelectStatementStandalone(
      ctx: TSqlParser.SelectStatementStandaloneContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitSelectStatement(ctx: TSqlParser.SelectStatementContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitQueryExpression(ctx: TSqlParser.QueryExpressionContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitQuerySpecification(ctx: TSqlParser.QuerySpecificationContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitSelectOptionalClauses(ctx: SelectOptionalClausesContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitFromClause(ctx: FromClauseContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitTableName(ctx: TableNameContext): ir.WithKnownOrigin[ir.LogicalPlan] = withOriginFromContext(ctx) {
    errorCheck(ctx) match {
      case Some(errorResult) => errorResult
      case None =>
  ...
  override def visitTableSource(ctx: TableSourceContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
          ...
          ctx match {
            case c if c.joinPart() != null => c.joinPart().asScala.foldLeft[ir.LogicalPlan](left)(buildJoinPart)
          }
      }
    }

  override def visitTableSourceItem(ctx: TableSourceItemContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitTsiNamedTable(ctx: TsiNamedTableContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitTsiDerivedTable(ctx: TsiDerivedTableContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitDerivedTable(ctx: DerivedTableContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitTableValueConstructor(ctx: TableValueConstructorContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitMerge(ctx: MergeContext): ir.WithKnownOrigin[ir.LogicalPlan] = withOriginFromContext(ctx) {
    errorCheck(ctx) match {
      case Some(errorResult) => errorResult
      case None =>
        ...
        val sourcePlan = tableSourcesPlan.tail.foldLeft[ir.LogicalPlan](tableSourcesPlan.head)(
  ...
  override def visitUpdate(ctx: UpdateContext): ir.WithKnownOrigin[ir.LogicalPlan] = withOriginFromContext(ctx) {
    errorCheck(ctx) match {
      case Some(errorResult) => errorResult
      case None =>
        ...
        val sourceRelation = tableSourcesOption.map { tableSources =>
          tableSources.tail.foldLeft[ir.LogicalPlan](tableSources.head)(
            ir.Join(_, _, None, ir.CrossJoin, Seq(), ir.JoinDataType(is_left_struct = false, is_right_struct = false)))
        }
  ...
  override def visitDelete(ctx: DeleteContext): ir.WithKnownOrigin[ir.LogicalPlan] = withOriginFromContext(ctx) {
    errorCheck(ctx) match {
      case Some(errorResult) => errorResult
      case None =>
        val target = ctx.ddlObject().accept(this)
  ...
  override def visitInsert(ctx: InsertContext): ir.WithKnownOrigin[ir.LogicalPlan] = withOriginFromContext(ctx) {
    errorCheck(ctx) match {
      case Some(errorResult) => errorResult
      case None =>
        val target = ctx.ddlObject().accept(this)
  ...
  override def visitInsertStatementValue(ctx: InsertStatementValueContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitOutputClause(ctx: OutputClauseContext): ir.WithKnownOrigin[ir.LogicalPlan] =
    withOriginFromContext(ctx) {
      errorCheck(ctx) match {
        case Some(errorResult) => errorResult
        case None =>
  ...
  override def visitDdlObject(ctx: DdlObjectContext): ir.WithKnownOrigin[ir.LogicalPlan] = withOriginFromContext(ctx) {
    errorCheck(ctx) match {
      case Some(errorResult) => errorResult
      case None =>
        ctx match {
